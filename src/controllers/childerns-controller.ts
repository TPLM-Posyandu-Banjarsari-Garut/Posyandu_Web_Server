...
//         logger.info({ childId: child.id }, 'Children record created successfully')
//         return ApiResponse.created(res, 'Children registered successfully', child)
//     }

//     getChildrens = async (req: Request, res: Response) => {
//         const user = req.user // untuk error ini ada hubungannya
//         const query = req.query as unknown as ChildrenQueryFilters

//         logger.info({ userId: user?.id, query }, 'Incoming request: Get Childrens')
...
//         logger.info({ userId: user?.id, public_id }, 'Incoming request: Get Children By ID')

//         if (user?.role === 'parent') {
//             await this.children_service.verifyParentAccess(public_id, user.parent_id)
//         }
...
//         const child = await this.children_service.updateChildren(public_id, req.body)
...
//         const child = await this.children_service.deleteChildren(public_id, is_permanent)
